class RecipesController < ApplicationController
  ...
  get "/recipes" do
    ...
    @recipes.each do |recipe|
      @Recipes[recipe] = Recipe.find_by(:recipe_id => recipe.id)
    end
    if logged_in?
      @user = current_user
  ...
  post "/recipes/:id/Recipe" do 
    @user = current_user
    @recipe = Recipe.find_by_id(params["id"])
    if !params["Recipe"].empty?
      @Recipe = Recipe.create(:Recipe => params["Recipe"], :recipe_id => @recipe.id, :user_id => @user.id)
      redirect "/recipes/#{@recipe.id}"
    else
      redirect "/recipes/#{@recipe.id}"
    end
